...
resource "azurerm_public_ip" "pip_udemy_web_server" {
  name                = "${var.prefix}-${var.udemy_web_server}-${format("%02d",count.index)}-pip"
  location            = azurerm_resource_group.rg_udemy.location
  resource_group_name = azurerm_resource_group.rg_udemy.name
  #condional example (true or false) - if production then Static else Dynamic
  allocation_method   = "Static"
  domain_name_label = "${var.prefix}-${var.udemy_web_server}-${format("%02d",count.index)}-dns"  
  count               = var.udemy_web_server_count
...
resource "azurerm_lb" "lb_udemy_webserver" {
  name                = "${var.prefix}-${var.udemy_web_server}-${format("%02d",count.index)}-lb"
  location            = azurerm_resource_group.rg_udemy.location
  resource_group_name = azurerm_resource_group.rg_udemy.name
  count               = var.udemy_web_server_count
  ...
  frontend_ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.pip_udemy_web_server[count.index].id
  }
}

resource "azurerm_lb_backend_address_pool" "lbbe_udemy_webserver" {
    resource_group_name = azurerm_resource_group.rg_udemy.name
    loadbalancer_id     = azurerm_lb.lb_udemy_webserver[count.index].id
    name                = "BackEndAddressPool"
    count = 2
}

resource "azurerm_network_interface_backend_address_pool_association" "lbbepool_udemy_webserver" {
    network_interface_id    = azurerm_network_interface.nic_udemy_web_server[count.index].id
    #ip_configuration_name   = "ipConfiguration"
    ip_configuration_name    = "TestConfiguartion1"
    backend_address_pool_id = azurerm_lb_backend_address_pool.lbbe_udemy_webserver[count.index].id 
    count = 2  
}
...
resource "azurerm_network_interface_nat_rule_association" "natasc_udemy_webserver" {
  network_interface_id  = azurerm_network_interface.nic_udemy_web_server[count.index].id
  ip_configuration_name = "TestConfiguartion1"
  nat_rule_id           = azurerm_lb_nat_rule.nat_udemy_webserver[count.index].id
  count = 2
}
